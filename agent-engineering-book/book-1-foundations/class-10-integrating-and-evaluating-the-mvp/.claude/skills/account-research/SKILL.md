...
Superseded as of Chapter 7 by two narrower Skills: `company-profiler`
(stable business facts) and `signal-hunter` (timely developments), both of
which call the real tools from Chapter 6 and attach claim-level, dated
citations per `config/evidence-policy.yaml`. This Skill is kept as the
original Chapter 4 reference implementation — plain-string evidence, no
tool calls, no fact/inference/hypothesis discipline — not as the Skill new
research should invoke.
