...
Currently at Chapter 5 (Structured Outputs and Data Contracts). One Skill
exists — `.claude/skills/account-research/` — invoked directly. The
versioned Account Brief contract now exists at
`schemas/account_brief.schema.json`, but the Skill does not yet produce
output in that shape — Chapter 7 wires research into it. No subagents,
tools, memory, or planning yet.
